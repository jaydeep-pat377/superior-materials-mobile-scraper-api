...
async function canReadDashboard(userId, id) {
  const { rows: dashRows } = await pool.query(
    'SELECT user_id, is_public FROM ai_dashboards WHERE id = $1 LIMIT 1',
    [id],
  );
...
export async function listComments(userId, id, widgetId) {
  ...
  let sql = 'SELECT id, widget_id, user_id, body, parent_id, created_at FROM ai_widget_comments WHERE dashboard_id = $1';
  const params = [id];
  if (widgetId) {
    sql += ' AND widget_id = $2';
...
export async function addComment(userId, id, { widgetId, body, parentId } = {}) {
  ...
  const { rows } = await pool.query(
    'INSERT INTO ai_widget_comments (dashboard_id, widget_id, user_id, body, parent_id) VALUES ($1, $2, $3, $4, $5) RETURNING id, widget_id, user_id, body, parent_id, created_at',
...
export async function editComment(userId, commentId, body) {
  ...
  const { rows } = await pool.query(
    'UPDATE ai_widget_comments SET body = $1 WHERE id = $2 AND user_id = $3 RETURNING id, body, updated_at',
