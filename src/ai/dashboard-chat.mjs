 ...
 * backend's JWT auth middleware (req.user.id).
 */
    ...
    onFinish: async (event) => {
      ...
      try {
        ...
        pool.query(
          `INSERT INTO ai_token_usage
             (user_id, thread_id, model_id, question, input_tokens, output_tokens, total_tokens, estimated_cost)
      ...
      try {
        const { rows: threadRows } = await pool.query(
          'SELECT title FROM ai_chat_threads WHERE id = $1',
          [threadId],
        );
