...
async function loadOrComputeBaseline(body) {
  ...
  const { rows: cachedRows } = await pool.query(
    'SELECT baseline_mean, baseline_stddev, sample_size, computed_at FROM ai_metric_baselines WHERE metric_key = $1 LIMIT 1',
    [key],
  );
  ...
  await pool.query(
    `INSERT INTO ai_metric_baselines (metric_key, table_name, group_by, method, value_col, baseline_mean, baseline_stddev, sample_size, computed_at)
    ...
    [key, body.table, body.groupBy ?? null, body.method, body.valueColumn ?? null, mean, stddev, values.length, new Date().toISOString()],
  );
