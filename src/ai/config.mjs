 ...
 * model, and an optional monthly token budget) via direct PostgreSQL queries.
 *
...
export async function getAiConfig() {
  try {
    const { rows } = await pool.query(
      "SELECT enabled_model_ids, default_model_id, monthly_token_budget, budget_enforced FROM ai_config WHERE id = 1 LIMIT 1",
    );
...
async function readProviderKeyRow() {
  try {
    const { rows } = await pool.query(
      "SELECT google_api_key, anthropic_api_key, azure_api_key, azure_resource_name, azure_deployment FROM ai_provider_keys WHERE id = 1 LIMIT 1",
    );
    return rows[0] || null;
  } catch {
    return null;
...
export async function getMonthToDateTokens() {
  try {
    ...
    const { rows } = await pool.query(
      "SELECT COALESCE(SUM(total_tokens), 0) AS total FROM ai_token_usage WHERE created_at >= $1",
      [monthStart],
    );
...
export async function getTokenBank(
  ...
  try {
    const { rows } = await pool.query(
      "SELECT scope, scope_id, monthly_allotment, bonus_tokens, enforced, period_start FROM ai_token_bank WHERE scope = $1 AND scope_id = $2 LIMIT 1",
      [scope, scopeId],
    );
    const data = rows[0] || null;
    if (!data) return null;
    ...
    if (storedPeriod && storedPeriod < periodStart) {
      ...
      bonus = 0;
      const setClauses = ["period_start = $1"];
      const params = [periodStart];
      if (resetBonus) {
        setClauses.push("bonus_tokens = 0");
      }
      void pool.query(
        `UPDATE ai_token_bank SET ${setClauses.join(", ")} WHERE scope = $${params.length + 1} AND scope_id = $${params.length + 2}`,
