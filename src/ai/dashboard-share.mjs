...
async function ownsDashboard(userId, id) {
  const { rows } = await pool.query(
    'SELECT id FROM ai_dashboards WHERE id = $1 AND user_id = $2 LIMIT 1',
    [id, userId],
  );
...
export async function getShareInfo(userId, id) {
  ...
  const [sharesRes, dashRes] = await Promise.all([
    ...
    pool.query(
      'SELECT share_token, is_public FROM ai_dashboards WHERE id = $1 LIMIT 1',
      [id],
    ),
  ]);
  const dash = dashRes.rows[0] || null;
  return {
...
export async function getPublicDashboard(token) {
  ...
  const { rows } = await pool.query(
    'SELECT id, title, layout, widgets, updated_at FROM ai_dashboards WHERE share_token = $1 AND is_public = true LIMIT 1',
    [token],
  );
