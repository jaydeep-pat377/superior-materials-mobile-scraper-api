  ...
  try {
    const { rows } = await pool.query(
      `SELECT enabled_model_ids, default_model_id, monthly_allotment, bonus_tokens,
       ...
       FROM ai_settings WHERE user_id = $1 LIMIT 1`,
      [userId],
    );
...
export async function updateAiSettings(userId, input = {}) {
  ...
  try {
    ...
    if (input.addBonusTokens && input.addBonusTokens > 0) {
      const currentPeriod = currentPeriodStart();
      const { rows } = await pool.query(
        'SELECT bonus_tokens, token_period_start FROM ai_settings WHERE user_id = $1 LIMIT 1',
        [userId],
      );
      const data = rows[0] || null;
    ...
    // Build dynamic upsert: INSERT ... ON CONFLICT (user_id) DO UPDATE SET ...
    ...
    try {
      await pool.query(
        `INSERT INTO ai_settings (${cols.join(', ')}) VALUES (${placeholders})
