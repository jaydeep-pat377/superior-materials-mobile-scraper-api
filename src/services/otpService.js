...
// ---------------------------------------------------------------------------
// PostgreSQL OTP table helpers
// Table: signup_otps (public schema)
...
async function checkSendLimits(identifier, type) {
  ...
  const { rows: countRows } = await pool.query(
    `SELECT COUNT(*) AS cnt FROM signup_otps
...
async function storeOtp(identifier, type, otp) {
  ...
  const { rowCount } = await pool.query(
    `INSERT INTO signup_otps (identifier, type, otp_hash, expires_at, attempts, verified)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [identifier, type, hashOtp(otp), expiresAt, 0, false]
  );

  if (rowCount === 0) {
    console.error('Error storing OTP: no rows inserted');
